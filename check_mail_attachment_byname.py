...
- 같은 파일명이 있으면 수신일시(_yymmdd_HHMM) 붙여 저장
  ※ 이 폴더를 일정 반영 도구(data-preprocessing repo 의 update_schedule*.py)가 소스로 읽는다.
  시각까지 붙어야 같은 날 두 번 온 파일의 최신 판정이 갈린다.
...
for mail in inbox.Items:
    ...
    # 이름 충돌로 스킵되어 유실 + 일정 반영 도구의 최신 판정이 동점 → 시각(_HHMM)까지 (2026-07-22)
